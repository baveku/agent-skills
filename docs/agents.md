...
## How personas relate to skills and entry points
...
| **Entry point** | A user-facing command, prompt, or harness action | `/review`, "review this PR" | The *when* — composes personas and skills |

The user, a slash command, or a commandless harness workflow is the orchestrator. **Personas do not call other personas.** Skills are mandatory hops inside a persona's workflow.
...
### Saved entry point (single persona behind it)
Pick this when there's a repeatable workflow you'd otherwise re-explain every time.

- `/review`, or "run a review workflow" → wraps `code-reviewer` with the project's review skill
- `/test`, or "design the missing tests" → wraps `test-engineer` with TDD skill
- `/webperf`, or "audit this page's Core Web Vitals" → wraps `web-performance-auditor` for performance-focused audits on web apps

### Orchestrated entry point (fan-out)
Pick this only when **independent** investigations can run in parallel and produce reports that a single agent then merges.

- `/ship`, or "ship-check this production change" → fans out to `code-reviewer` + `security-auditor` + `test-engineer` in parallel when the harness supports subagents, then synthesizes their reports into a go/no-go decision
- In harnesses without subagents or slash commands, run the same perspectives sequentially and merge the reports in the main session.
...
└── No  → Are the sub-tasks independent (no shared mutable state, no ordering)?
         ├── Yes → Orchestrated fan-out when the harness supports it (e.g. /ship)
         └── No  → Sequential lifecycle workflow run by the user or AGENTS.md routing
```
...
Commandless equivalent:

```
"Ship-check this production change"
...
- If the harness cannot run parallel subagents, the same reports may be produced sequentially. That loses wall-clock speed, not the review model.
...
- The user already knows they want a review; let them invoke `code-reviewer`, `/review`, or the equivalent natural-language workflow directly
- Replicates work that entry points and `AGENTS.md` intent-mapping already do
...
2. **Personas do not invoke other personas.** Composition is the job of entry points or the user. On Claude Code this is also a hard platform constraint — *"subagents cannot spawn other subagents"* — so the rule is enforced for you.
...
- **As subagents:** auto-discovered when this plugin is enabled (no path config needed). Use the Agent tool with `subagent_type: code-reviewer` (or `security-auditor`, `test-engineer`). `/ship` is the canonical example when slash commands are available; commandless harnesses can request the same fan-out in natural language.
